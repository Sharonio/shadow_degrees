...
yCrop = imcrop(new_img_gray,[min(corners) max(corners)]);
...
interest_area_img = imwarp(yCrop,tform,'OutputView', imref2d(size(yCrop)));
...
xlim( mean(cur_xlim) + 1.0*(cur_xlim-mean(cur_xlim))-[0 1]  );
ylim( mean(cur_ylim) + 1.0*(cur_ylim-mean(cur_ylim)) - [0 1] );
