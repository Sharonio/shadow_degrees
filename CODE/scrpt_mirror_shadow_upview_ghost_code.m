% scrpt_mirror_shadow_upview_ghost_code
...
if (show_ghost_flg==1)
   circle_x = ( cur_xlim(1) + 0.75*( cur_xlim(2)-cur_xlim(1) ) ) +...
       5*randn(1)+ 100*cos( linspace(0,2*pi,40) );
   circle_y = mean( cur_ylim ) + 5*randn(1) + 100*sin( linspace(0,2*pi,40) );
   patch( circle_x , circle_y ,'k' );
end


% scaling_factor = scaling_factor + scaling_factor_direction*0.1;
% if scaling_factor>=2
%     scaling_factor_direction = -1;
% end
% if scaling_factor <=0.5
%     scaling_factor_direction = 1;
% end

% xlim( mean(cur_xlim) + 1.6*(cur_xlim-mean(cur_xlim))  );
% ylim( 40+mean(cur_ylim) + 1.7*(cur_ylim-mean(cur_ylim))  );

xlim(  mean(cur_xlim) + scaling_factor*(cur_xlim-mean(cur_xlim))  );
ylim(  mean(cur_ylim) + scaling_factor*(cur_ylim-mean(cur_ylim))  );
